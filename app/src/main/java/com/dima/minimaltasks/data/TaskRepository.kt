...
class TaskRepository(
    ...
    suspend fun deleteTask(taskId: String): Boolean = database.withTransaction { taskDao.deleteById(taskId) == 1 }
    ...
    suspend fun undoCompletion(taskId: String, nowMillis: Long = System.currentTimeMillis()): UndoCompletionResult? =
        database.withTransaction {
            ...
            val removedChildId = taskDao.findChildFor(taskId)?.id
            taskDao.deleteChildFor(taskId)
            UndoCompletionResult(
                restoredTask = task.copy(completed = false, completedAt = null, updatedAt = nowMillis),
                removedChildId = removedChildId,
            )
        }
